...
int utils::Zipper::zip(const std::string& source_filename, const std::string& dest_filename)
{
	FILE* source = fopen(source_filename.c_str(),"r");
	if(source == NULL)
	  throw ZipperException("zip: could not open " + source_filename,DavidException::IO_ERROR_CODE);

	FILE* destination = fopen(dest_filename.c_str(),"w");
	if(destination == NULL)
		throw ZipperException("zip: could not write to " + dest_filename,DavidException::IO_ERROR_CODE);
...
int utils::Zipper::fzip(FILE *source, FILE *dest, int level)
{
    int ret, flush;
    unsigned have;
    z_stream strm;
...
int utils::Zipper::unzip(const std::string& source_filename, const std::string& dest_filename)
{
	FILE* source = fopen(source_filename.c_str(),"r");
	if(source == NULL)
		throw ZipperException("unzip: could not open " + source_filename,DavidException::IO_ERROR_CODE);

	FILE* destination = fopen(dest_filename.c_str(),"w");
	if(destination == NULL)
		throw ZipperException("unzip: could not write to " + dest_filename,DavidException::IO_ERROR_CODE);
...
int utils::Zipper::funzip(FILE *source, FILE *dest)
{
    int ret;
    unsigned have;
    z_stream strm;
...
void utils::Zipper::zerr(int ret)
{
    fputs("zpipe: ", stderr);
    switch (ret) {
    case Z_ERRNO:
...
void utils::Zipper::write_oct(char* buffer, const int& number, const int& width)
{
...
void utils::Zipper::write_dec(char* buffer, const int& number, const int& width)
{
  std::ostringstream buf;
  buf.width(width);
  buf.fill('0');
...
int utils::Zipper::checksum(const char* header,const size_t& header_size)
{
  int sum = 0;
...
std::stringstream* utils::Zipper::funtar(FILE* in_file,const std::string& filename)
{
	...
	while(!feof(in_file))
	{
		...
		if(int_buff.fail())
		  throw ZipperException("utils::Zipper::untar: Could not read file size from tar file.",DavidException::DATA_FORMAT_ERROR);
		if(block_filename == filename)//does it match what we're looking for. Note: prefix (i.e. subdirectory) is included in tar-header entry.
		{
			...
			if(amount_read != filesize)
				throw ZipperException("utils::Zipper::untar: Could not read entire buffer from tar file.",DavidException::DATA_FORMAT_ERROR);
...
void utils::Zipper::tar(const std::vector<std::string>& filenames,FILE* out_file,const std::string& dir,const std::string& prefix)
{

  if(filenames.size() == 0)
	  throw ZipperException("tar: At least one input file name is needed.");

  if(out_file == 0)
	  throw ZipperException("tar: the output file pointer is null.");
...
void utils::Zipper::pad_tarfile(const char* data_buffer, FILE* out_file)
{
...
struct stat utils::Zipper::default_stat(const size_t& file_size)
{
...
void utils::Zipper::create_header(char* header, const char* data, const size_t& data_size, const struct stat& file_stat, const char* data_filename)
{
