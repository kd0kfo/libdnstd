 ...
 *
 * Created by David Coss 2010, freely licensed under the GNU General Public License, http://gnu.org
 *
 */
...
class ZipperException : public DavidException
{
public:
	ZipperException(const std::string& error) : DavidException( error){}

	ZipperException(const std::string& error, const int& errorType)
	: DavidException(error,errorType){}
	ZipperException(const std::ostringstream& error, const int& errorType)
	: DavidException(error,errorType){}
	ZipperException(const std::ostringstream& error)
		: DavidException(error,UNKNOWN_ERROR){}
